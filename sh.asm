...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    if(fork1() == 0)
      ...
      runcmd(parsecmd(buf));
    wait();
      51:	e8 a4 0d 00 00       	call   dfa <wait>
  while(getcmd(buf, sizeof(buf)) >= 0){
  ...
  exit();
      9d:	e8 50 0d 00 00       	call   df2 <exit>
      buf[strlen(buf)-1] = 0;  // chop \n
      ...
      if(chdir(buf+3) < 0)
      bc:	e8 a1 0d 00 00       	call   e62 <chdir>
      ...
      e9:	e8 2c 0d 00 00       	call   e1a <close>
      ee:	66 90                	xchg   %ax,%ax
      break;
...
000001b0 <runcmd>:
{
  ...
  switch(cmd->type){
    ...
    wait();
     213:	e8 e2 0b 00 00       	call   dfa <wait>
    wait();
     218:	e8 dd 0b 00 00       	call   dfa <wait>
     21d:	8d 76 00             	lea    0x0(%esi),%esi
      exit();
     220:	e8 cd 0b 00 00       	call   df2 <exit>
    if(fork1() == 0)
    ...
    exec(ecmd->argv[0], ecmd->argv);
     ...
     243:	e8 e2 0b 00 00       	call   e2a <exec>
    ...
    wait();
     272:	e8 83 0b 00 00       	call   dfa <wait>
    runcmd(lcmd->right);
     ...
     cd5:	31 f6                	xor    %esi,%esi
{
  ...
  for(i=0; i+1 < max; ){
    ...
    cc = read(0, &c, 1);
     ...
     cf3:	e8 12 01 00 00       	call   e0a <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
  ...
  fd = open(n, O_RDONLY);
     ...
     d46:	e8 e7 00 00 00       	call   e32 <open>
  if(fd < 0)
  ...
  r = fstat(fd, st);
     ...
     d5b:	e8 ea 00 00 00       	call   e4a <fstat>
  close(fd);
     d60:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
     d63:	89 c6                	mov    %eax,%esi
  close(fd);
     d65:	e8 b0 00 00 00       	call   e1a <close>
  return r;
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  write(fd, &c, 1);
     ...
     f1a:	e8 f3 fe ff ff       	call   e12 <write>
  while(--i >= 0)
     ...
     f4e:	8d 45 10             	lea    0x10(%ebp),%eax
{
  ...
  write(fd, &c, 1);
     f86:	e8 87 fe ff ff       	call   e12 <write>
  ...
  write(fd, &c, 1);
     ...
     ff6:	e8 17 fe ff ff       	call   e12 <write>
        putc(fd, c);
  ...
  write(fd, &c, 1);
    1013:	e8 fa fd ff ff       	call   e12 <write>
  for(i = 0; fmt[i]; i++){
  ...
  write(fd, &c, 1);
    ...
    10a0:	e8 6d fd ff ff       	call   e12 <write>
        while(*s != 0){
  ...
  write(fd, &c, 1);
    ...
    10d0:	e8 3d fd ff ff       	call   e12 <write>
  ...
  write(fd, &c, 1);
    ...
    1120:	e8 ed fc ff ff       	call   e12 <write>
        ap++;
...
malloc(uint nbytes)
{
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    122a:	e8 4b fc ff ff       	call   e7a <sbrk>
  if(p == (char*)-1)
